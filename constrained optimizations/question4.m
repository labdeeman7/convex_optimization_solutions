...
N=2^(13); 
T_size = 100; %%% amount of randomly distributed spikes 
K=2^(10);
...
%%% note this is a one dimensional signal
[nx,ny] = size(x);

%% Measurement type Gaussian random matrix. 

A_gaussian=orth(randn(N))'; %%% turn to orthonormal rows
A_gaussian=A_gaussian(1:K,:); %%%Ensures that A is kxn
gClean = A_gaussian*x;

% Add noise gaussian with sd of 0.005
sigma_noise = 0.005;
gNoisy = gClean + sigma_noise * randn(size(gClean));

%%%Can I display the original signal, the gaussian transformed one and the
%%%noisy signal?

%% Operator definition for gaussians

A = @(x) A_gaussian*x;
At = @(x) A_gaussian'*x;
AtA = @(x) At(A(x));  

%% Measurement type Welsh hadamard transform.
% Do this later.
% S=eye(N);
